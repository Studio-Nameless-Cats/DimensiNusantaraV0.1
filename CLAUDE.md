...
- `Data/` — `CharacterData` (+ MaxMp, Skills/SpecialSkills lists, **Level & Growth**: startingLevel/expReward/per-stat growth), `SkillData` (+ `unlockLevel`), `ItemData` (category OBAT/PERANG/BAHAN, use effect, equip slot + stat bonuses), `LevelCurve` (static EXP curve), `EnemyEncounterData`, `EnemyAIData`, `WorldMarkerData`, `GameDatabase` (Characters + Items id lookups) (ScriptableObjects)
- `Items/` — `InventorySystem` (on Player, next to PartySystem): the bag ({ItemData, count} stacks, capacity) + per-character equipment (Senjata/Zirah/Jimat x2, characterId -> Equipment); static data survives scene reloads; saves as ISaveParticipant module "inventory"; static `*BonusFor(characterId)` feeds gear bonuses into `PartyMember` stats
- `UI/Menu/` (inventory bits) — `InventoryPanel` (INVENTARIS controller: tabs, chip grid, tooltip, Z PAKAI / X BUANG, Q/E switcher, stat strip), `InventoryItemChip`, `EquipPicker`/`EquipPickerRow` (slot-click modal w/ stat deltas + Lepas)

Resources: party members have **MP** (persists in SaveData, restored on rest) and a per-battle **Special gauge** (0..100, resets each battle, fills via attacks/taking hits). Skills cost MP; special skills cost the gauge. **Level/EXP:** members have a level + EXP (persisted, save v4); winning a battle awards EXP (sum of defeated enemies' `expReward`) to survivors → level-up grows stats (`base + growth·(level-1)`) and unlocks skills (`level >= SkillData.unlockLevel`). EXP curve in `LevelCurve.cs`. EXP bar shows in the Character menu tab; no MP/Special/EXP meters in the battle HUD yet. **Items/equipment:** per-character gear bonuses are added inside `PartyMember.Attack/Defense/Speed`, so battle gets them for free; consumables usable from the inventory panel only (no battle item command yet). Inventory panel editor wiring pending per `GUIDE_Inventory_Editor.md` (Cowork project).
